...
def usr_args():
    ...
    optional.add_argument(
        "-o", "--output", help="Output file to create. Default is a JSON file. "
    )
    optional.add_argument(
        "-m",
        "--multi",
...
def sheet_2_json(file_path):
    ...
    if extension == "tsv":
        ...
        print(
            "Must supply a valid 'TSV' or 'CSV'",
...
def validate_schema(in_file, schema_file, out_file):
    ...
    for line in json_list:
        count += 1
        line_errors = {"line_number": count, "failed_cells": []}
        # try:
        ...
        for item in errors:
            try:
                line_errors["failed_cells"].append(
                    f"{item.relative_path[0]}: {item.message}"
                )
            except IndexError:
    ...
    if out_file:
        with open(out_file, "w", encoding="utf-8") as file:
            json.dump(error_strings, file, sort_keys=True, indent=4, ensure_ascii=False)
    else:
    ...
    for item in schema_list:
        ...
        for sheet in input_list:
            if search_term in sheet:
                count += 1
                output_file = (
                    output_dir + "/" + sheet.split("/")[-1].split(".")[0] + ".json"
                )
