...
# MGRC GREGoR Dashboard AWS Deployment Instructions & Notes
...
3. These instructions pertain to installing an instance of the GREGoR Dashboard on a Amazon Linux 2023 machine which will have its ports opened to allow access from outside the network (detailed below).  If you do not want to allow the outside world (e.g. anything outside of the LAN that the server exists in) to access the server then simply ignore the steps for opening up the ports.  If you DO want to allow the outside world to access your server, it is likely that your sysadmin or IT department will have to configure additional firewall/DNS settings to allow the connections.

## Installation on an Amazon Linux 2023 Instance

### A note about instance type

These instructions are for the <a href="https://aws.amazon.com/marketplace/pp/prodview-zc4x2k7vt6rpu?sr=0-2&ref_=beagle&applicationId=AWSMPContessa" target="_blank">Amazon Linux 2 AMI</a> image hosted on AWS EC2.

### 1. Create users and directories

#### Users

First we need to create a user, gregor_user, specifically for running the dashboard, 

```
sudo useradd gregor_user
```

then we need to set the password, 

```
sudo passwd gregor_user
```

Now that we have our gregor user, we need to add them to the group that is allowed to operate with root permissions, 

```
sudo usermod -a -G wheel gregor_user
```
...
**Purpose of /home/gregor_user/GitHub/:**  The directory to house the source code clone from GitHub.

Create the directory, 

```
mkdir /home/gregor_user/GitHub/
```

**/var/www/dashboard/**
**Purpose:**  The directory to house the **built** frontend for the Dashboard.
...
Finally, install the Python developer packages,
